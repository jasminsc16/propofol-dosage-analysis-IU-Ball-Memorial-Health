...
## Visual comparison of IU Health Hospitals
...
## Visualizing selected variables from EDA

The target variable I have chosen to evaluate propofol usage is Per_prop_over50, which is the % of time of propofol above 50 mcg/kg/min. After evaluating the EDA results, the selected covariates (explanatory variables) are: Home_Opiod, Home_Benzo, Opiate_Tol, Prop_dec_afterDex, Fent_before_prop, Prop_dec_after_fent, Was_prop_dec, Rec_pain_med, Con_Prop_Vas, Con_Prop_Dex, Con_Prop_Fen, Con_Prop_Ket, Con_Prop_Ben, and Con_Prop_NMBA. Below are some additional visualizations showing covariation between per_prop_over50 and the selected covariates.
...
## Evaluating evidence of interaction


There seems to be potential evidence of interaction between the total days on propofol infusion (Total_days_prop) (quantitative) and the organization (Org_Name) (qualitative).
